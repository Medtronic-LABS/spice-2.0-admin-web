...
import * as regionService from '../../services_com/regionAPI';
...
*/
function* regionSagaCom() {
  yield all([takeLatest(USERTYPES.UPLOAD_FILE_REQUEST_COM, uploadFileSaga)]);
  yield all([takeLatest(USERTYPES.DOWNLOAD_FILE_REQUEST_COM, downloadFileSaga)]);
  yield all([takeLatest(USERTYPES.REGION_DETAILS_REQUEST_COM, regionDetailsSaga)]);
}

export default regionSagaCom;
