...
describe('SelectFieldWrapper', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = mount(<Form onSubmit={() => {}}>{() => <SelectFieldWrapper {...props} />}</Form>);
  });
  ...
  it('should be a function that returns null when inputProps.disabledValidation is truthy', () => {
    const componentWrapper = mount(
      // tslint:disable-next-line:no-empty
      <Form onSubmit={() => {}}>{() => <SelectFieldWrapper obj={{}} inputProps={{ disabledValidation: true }} />}</Form>
    );
    const validateProp = componentWrapper.find(Field).prop('validate');
    if (validateProp) {
