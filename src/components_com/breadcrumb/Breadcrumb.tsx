...
import { healthFacilitySelector } from '../../store/healthFacility_com/selectors';
import { clearHealthFaciliityDetail } from '../../store/healthFacility_com/actions';
 ...
 */
const BreadcrumbCommunity = (): React.ReactElement => {
  ...
  const sections: ISection[] = useMemo(() => {
    ...
    if (region?.name && region.tenantId && showRegion) {
      result.push({
        label: region.name,
    ...
    (label: any, routeInitArray?: string[], currentRouteArr?: any[]) => {
      ...
      if (routeInitArray && currentRouteArr && routeInitArray.length === currentRouteArr.length) {
        ...
        if (routeObject.name === APPCONSTANTS.ROUTE_NAMES.REGION) {
          dispatch(setRegionDetailCom(dispatchData(routeObject, label)));
        }
...
export default BreadcrumbCommunity;
