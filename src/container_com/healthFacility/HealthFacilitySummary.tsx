...
import CustomTable from '../../components_com/customTable/CustomTable';
...
import {
  ...
  IPeerSupervisor,
  IVillages
} from '../../store/healthFacility_com/types';
import {
  healthFacilityLoadingSelector,
  healthFacilitySelector,
  userDetailLoadingSelector
} from '../../store/healthFacility_com/selectors';
import { emailSelector, getAppTypeSelector, roleSelector } from '../../store/user/selectors';
...
export const formatHFUserData = (userData: any[], countryId: number | string, tenantId?: number | string) =>
  userData.map((user: any) => {
    ...
    if (user?.healthFacility?.tenantId) {
      newtenantId = Number(user.healthFacility.tenantId);
    } else if (tenantId || user.tenantId) {
      newtenantId = Number(tenantId) || user.tenantId;
    } else if (user.organizations?.[0]?.id) {
      newtenantId = Number(user.organizations[0]?.id);
    }
    return {
      id: Number(user?.id),
      ...
      supersetUserOrganizationIds: (Array.isArray(user?.supersetUserOrganization)
        ? user.supersetUserOrganization
        : []
      ).map(({ tenantId: hfTenantId }: { tenantId: number }) => hfTenantId)
    };
  });

const HealthFacilitySummaryCom = (): React.ReactElement => {
  ...
  }, [appTypes, dispatch, healthFacilityId, hfTenantId]);
  ...
  const handleHFEditDetailsSubmit = ({ healthFacility: healthFacilityData }: { healthFacility: IHealthFacility }) => {
    const postData = formatHealthFacility(healthFacilityData, countryId);
  ...
  const handleEditUserSubmit = ({ users }: { users: any[] }) => {
    const userObj = formatHFUserData(users, countryId, hfTenantId);
...
export default HealthFacilitySummaryCom;
