...
import { IHealthFacility, IHealthFacilityForm } from '../../store/healthFacility_com/types';
...
const HealthFacilityListCom = (): React.ReactElement => {
  ...
  }, [countryId, dispatch, isSuperUser, listParams.page, listParams.rowsPerPage, listParams.searchTerm]);
  ...
  const openCreateHealthFacility = () => {
    ...
    history.push(url.replace(':regionId', String(countryId)));
  };

  const handleRowClick = (data: any) => {
    history.push(
      PROTECTED_ROUTES.healthFacilitySummaryCom
        .replace(':healthFacilityId', data.id)
        .replace(':hfTenantId', data.tenantId)
    );
  };
...
export default HealthFacilityListCom;
