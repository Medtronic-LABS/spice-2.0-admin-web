...
import Loader from '../../components_com/loader/Loader';
...
import { IHFUserGet, IHFUserPost, IUserRole } from '../../store/healthFacility_com/types';
import CustomTable from '../../components_com/customTable/CustomTable';
import { emailSelector, roleSelector } from '../../store/user/selectors';
import {
  clearSupervisorList,
  clearVillageHFList,
  createHFUserRequest,
...
const UserListCom = (props: IMatchProps): React.ReactElement => {
  ...
  // const regionData = useSelector(getRegionDetailSelector);
  ...
  const refreshHFUserList = useCallback(
    ...
    [dispatch, listParams.page, listParams.rowsPerPage, listParams.searchTerm, countryId, role]
  );
  ...
  const handleUserSubmit = useCallback(
    ({ users }: { users: IHFUserGet[] }) => {
      const userObj = formatHFUserData(users, countryId, tenantId);
    ...
    [isOpenUserModal.isEdit, onSubmitHandler, countryId, siteUserSuccess, tenantId]
  );
