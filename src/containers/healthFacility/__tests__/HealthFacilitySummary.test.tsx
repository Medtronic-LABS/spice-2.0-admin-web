...
describe('SiteSummary', () => {
  ...
  it('should handle render methods in ModalForm', () => {
    const ModalFormWrapper = wrapper.find('Memo()').at(1);
    const handleCancel = ModalFormWrapper.prop('handleCancel');
    handleCancel();
    expect(wrapper.find('Memo()')).toHaveLength(2);
  });

  it('should handle render methods in ModalForm', () => {
    const ModalFormWrapper = wrapper.find('Memo()').at(0);
    const handleCancel = ModalFormWrapper.prop('handleCancel');
    handleCancel();
    expect(wrapper.find('Memo()')).toHaveLength(2);
  });
  ...
  it('should render siteUserFormRender', () => {
    const ModalFormWrapper = wrapper.find('Memo()').at(0);
    const render = ModalFormWrapper.prop('render');
    render();
  });
  it('should render siteUserFormRender', () => {
    const ModalFormWrapper = wrapper.find('Memo()').at(1);
  ...
  it('should render onFormSubmit', () => {
    const ModalFormWrapper = wrapper.find('Memo()').at(1);
    const handleFormSubmit = ModalFormWrapper.prop('handleFormSubmit');
    handleFormSubmit({ users: [''] });
  });
